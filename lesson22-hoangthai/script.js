...
const desInput = $('.add__description');
const valueInput = $('.add__value');
const incomeList = $('.income__list');
const expensesList = $('.expenses__list');
const budgetVal = $('.budget__value');
const budgetIncomeVal = $('.budget__income--value');
const budgetExpensesVal = $('.budget__expenses--value');
const budgetExpensesPer = $('.budget__expenses--percentage');
const budgetTotalVal = $('.budget__value');
const addChoise = $('.add__type');

addBtn.onclick = () => {
    if((desInput.value != '') && valueInput.value != '') {
        value = parseInt(valueInput.value, 10);
        if(addChoise.value === 'inc') {
            items.push({name : desInput.value, value : value});
        } else {
            items.push({name : desInput.value, value : (-1) * value});
        }
        loadData();
    }
}
...
function loadBudget() {
    ...
    budgetIncomeVal.innerHTML = '+' + income + '.00';
    let expense = items.reduce((initVal, item) => {
        if(item.value < 0) {
            return initVal + item.value;
        }
        return initVal;
    }, 0);
    budgetExpensesVal.innerHTML = expense + '.00';
    budgetExpensesPer.innerHTML = Math.round(expense*(-100)/income) + '%';
    remainVal = income + expense;
    
    budgetVal.innerHTML = ((remainVal > 0) ? '+' : '') + remainVal + '.00';
}